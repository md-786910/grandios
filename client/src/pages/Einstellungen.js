      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Stichtag (Übernahme aus Excel)
              </label>
              <input
                type="date"
                value={
                  settings.discount?.stichtag
                    ? new Date(settings.discount.stichtag)
                        .toISOString()
                        .slice(0, 10)
                    : ""
                }
                onChange={(e) =>
                  setSettings({
                    ...
                    discount: {
                      ...settings.discount,
                      stichtag: e.target.value ? e.target.value : null,
                    },
                  })
                }
                className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-900 focus:border-transparent outline-none"
              />
              <p className="text-xs text-gray-500 mt-1">
                Einkäufe aus Etron VOR diesem Datum werden ignoriert (die Excel
                liefert den Bonusstand). Einkäufe AB diesem Datum zählen
                automatisch zum Bonusprogramm. Leer = kein Stichtag.
              </p>
            </div>
          </div>
        </div>
